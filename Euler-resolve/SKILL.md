---
name: Euler-resolve
description: A structural framework for any visual/artistic task — painting, drawing, illustration, sketching, character or environment design, poster/editorial layout, branding and logo work, UI/UX and web design, or general "make this look better" requests. Combines mathematical composition frameworks (golden ratio, grids, symmetry, perspective), human visual perception and biology (Gestalt principles, eye anatomy, contrast sensitivity, color perception), classical artistic principles (balance, hierarchy, rhythm, unity, typography), and a "negative design" checklist of the specific mistakes AI-generated visuals commonly make. Use this whenever a request involves creating, critiquing, or improving anything visual — including when the user asks why a design "looks off," "looks AI-generated," or "looks amateur," even if they don't name a specific technique.
---

# Euler-resolve

*A structural framework for artistic and design hierarchy — math, perception, artistic principles, and where AI-generated work goes wrong.*
...
| `references/perception-and-biology.md` | You need to check *where the eye will actually go* and whether the hierarchy will physically read — Gestalt grouping, contrast, eye-scan patterns, depth cues, reading-direction defaults. |
| `references/artistic-principles.md` | You're deciding how to express the structure — balance, emphasis, rhythm, negative space, unity vs. variety, typography. |
...
0. **If this is a critique of existing work, make sure you can actually see it.** A request to critique or diagnose "why does this look off" only produces a real answer if you're looking at the actual piece. Check whether an image was genuinely uploaded (don't assume one exists just because the user is discussing it) before running the audit below — if there's nothing to look at, ask for the image or a specific enough description, rather than guessing at problems that may not be there.

1. **Identify the medium, audience, and constraints.** Static or interactive? Physical or digital? Fixed canvas or responsive? Reading direction of the actual audience (LTR or RTL — see `perception-and-biology.md`)? This determines which parts of `mathematical-foundations.md` even apply (e.g., WCAG contrast ratios matter for UI, not for oil painting).
...
6. **If the deliverable is code** (a web page, a UI component, a generative piece), the visual decisions above tell you *what* to build. *How* you build it follows the same spirit as `karpathy-guidelines` if that skill is available in this environment — minimum code that achieves the visual spec, no speculative configurability, surgical edits if you're modifying existing code. If you're working in this environment's React/HTML artifacts, also check `frontend-design` if available for the concrete CSS tokens and constraints here — this skill tells you the design should have an asymmetrical focal point in the upper third; `frontend-design` tells you which utility classes are actually available to build it. Neither reference is a hard dependency: if they aren't installed, apply the same underlying principle (minimum necessary code, no unrequested abstraction) directly.

## Success criteria

Before presenting finished visual work, you should be able to answer all of these:

- What is the focal point of this compositional unit, and what three things (contrast, position, isolation, scale, color, direction) make it read first?
